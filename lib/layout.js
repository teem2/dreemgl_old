...
define(function () {
	...
	function fillNodes(node) {
		...
		if(node._pos){
			if(!isNaN(node._pos[0]))	node.style.left = node._pos[0];
			if(!isNaN(node._pos[1]))	node.style.top = node._pos[1];
		}
		
		if (node._alignitems) node.style.alignItems = node._alignitems;
		if (node._aligncontent) node.style.alignContent = node._aligncontent;
		if (node._justifycontent) node.style.justifyContent = node._justifycontent;
		if (node._alignself) node.style.alignSelf = node._alignself;
		if (node._flexwrap) node.style.flexWrap = node._flexwrap;
		if (node.sizetocontent) node.style.measure = node.sizetocontent.bind(node);
		if (node._flexdirection) node.style.flexDirection = node._flexdirection;
		
		if (node._margin[0] == node._margin[1] == node._margin[2] == node._margin[3]) node.style.margin = node._margin[0];
		node.style.marginLeft = node._margin[0];
		node.style.marginRight = node._margin[2];
		node.style.marginTop = node._margin[1];
		node.style.marginBottom = node._margin[3];

		if (node._padding[0] == node._padding[1] == node._padding[2] == node._padding[3]) node.style.padding = node._padding[0];
		node.style.paddingLeft = node._padding[0];
		node.style.paddingRight = node._padding[2];
		node.style.paddingTop = node._padding[1];
		node.style.paddingBottom = node._padding[3];

		if (node._size){
			if (!isNaN(node._size[0]))	node.style.width = node._size[0];
			if (!isNaN(node._size[1]))	node.style.height = node._size[1];
		}
		if (node._position) node.style.position = node._position;
		if (node._flex) node.style.flex = node._flex;
...
//			  console.log("axis" , crossAxis, "should stretch!");
	...
	function layoutNode(node, parentMaxWidth, /*css_direction_t*/parentDirection) {
		...
		if (true){
			if (node.laststyle){
				if (Object.keys(node.laststyle).length == Object.keys(node.style).length) {
					skipLayout = false;
					for (a in node.laststyle)
					{
						if (node.style[a] != node.laststyle[a])
						{
							skipLayout = false;
						}else{
						}
			...
			if (node.dirty) skipLayout = false;
			if (layout.last_requested_width != node.style.width) skipLayout = false;
			if (layout.last_requested_height != node.style.height) skipLayout = false;
			if (layout.last_parent_max_width != parentMaxWidth)  skipLayout = false;
			if (layout.last_direction != direction) skipLayout = false;

			var actualskiplayout = !node.dirty && (layout.last_requested_width == node.style.width) && (layout.last_requested_height == node.style.height) && (layout.last_parent_max_width == parentMaxWidth) && (layout.last_direction == direction);
		}
		
		if (skipLayout) {
			layout.width = layout.last_width;
			layout.height = layout.last_height;
			layout.top = layout.last_top;
			layout.left = layout.last_left;
			layout.bottom = layout.last_bottom;
			layout.right = layout.last_right;
