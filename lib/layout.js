...
define(function () {
	...
	function fillNodes(realnode) {
		var node = {children:[]}

		var layout = realnode.layout = node.layout = {}
		...
		if(realnode._pos){
			if(!isNaN(realnode._pos[0]))	style.left = realnode._pos[0];
			if(!isNaN(realnode._pos[1]))	style.top = realnode._pos[1];
		}
		
		if (realnode._alignitems) style.alignItems = realnode._alignitems;
		if (realnode._aligncontent) style.alignContent = realnode._aligncontent;
		if (realnode._justifycontent) style.justifyContent = realnode._justifycontent;
		if (realnode._alignself) style.alignSelf = realnode._alignself;
		if (realnode._flexwrap) style.flexWrap = realnode._flexwrap;
		if (realnode.sizetocontent) style.measure = realnode.sizetocontent.bind(realnode);
		if (realnode._flexdirection) style.flexDirection = realnode._flexdirection;
		
		if (realnode._margin[0] == realnode._margin[1] == realnode._margin[2] == realnode._margin[3]) style.margin = realnode._margin[0];
		style.marginLeft = realnode._margin[0];
		style.marginRight = realnode._margin[2];
		style.marginTop = realnode._margin[1];
		style.marginBottom = realnode._margin[3];

		if (realnode._padding[0] == realnode._padding[1] == realnode._padding[2] == realnode._padding[3]) style.padding = realnode._padding[0];
		style.paddingLeft = realnode._padding[0];
		style.paddingRight = realnode._padding[2];
		style.paddingTop = realnode._padding[1];
		style.paddingBottom = realnode._padding[3];

		if (realnode._size){
			if (!isNaN(realnode._size[0]))	style.width = realnode._size[0];
			if (!isNaN(realnode._size[1]))	style.height = realnode._size[1];
		}
		if (realnode._position) style.position = realnode._position;
		if (realnode._flex) style.flex = realnode._flex;
	
		if(realnode.children) for(var i = 0; i < realnode.children.length;i++){
			var child = realnode.children[i]
			//if(child._mode && isNaN(child._flex)) continue
			node.children.push( fillNodes(child) )
		}
